...
- **File Blender (une seule instance à la fois, `-t 4`)** : profils et tubes (137 ; arrêtée à 61 le 15/09 à 0 h 54 par un manque de mémoire, relancée sur les 76 manquants) → tôles (39) → armatures (32) → vague 2 (139) → vague 2b relief et perforées (26) → vague 3 (80 : 68 visuels + 12 studios). Chaîne détachée de la séance (`chaine.ps1` du scratchpad, Blender lancé par `Start-Process`) ; fin écrite dans `rendu3d\chaine-terminee.txt`. Environ 3 min 30 par image : la file dure jusqu'au 15/09 au soir. **Pas de build Node pendant les rendus** (mémoire). À chaque fin de série : `habiller_famille.py`, vérification indépendante, intégrer, build, commit.
