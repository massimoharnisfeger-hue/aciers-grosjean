...
- **Cause** : le recadrage avait été ajouté comme un script à part, « une fois », au lieu d'être une étape de l'intégration. Toute famille réintégrée ensuite aurait réintroduit la fiche incrustée sur le site, sans qu'aucun contrôle ne le voie. Second piège, vu en corrigeant : le hook a refusé la ligne `import recadrer_visuels` pendant que l'appel `recadrer_visuels.CADRE` passait, et un contrôle qui cherchait seulement le mot restait vert sur un script qui aurait planté. Un contrôle de source vérifie la **structure** (l'import réel, l'appel dans la bonne fonction), pas la présence d'un mot.
  Réécrite le 24/09 : le propriétaire veut désormais l'image servie **entière**, fiche technique comprise (ADR-0012) ; la leçon tient (ce que le site sert se décide dans l'intégration, contrôlé par sa structure), le contrôle vérifie maintenant que `copier()` copie tel quel, sans aucun recadrage.
- **Contrôle** : `tests/test_catalogue.py::CatalogueSource::test_k12_l_integration_sert_l_image_entiere`
- **Date** : 2026-09-23
...
### L-063 — une image se juge telle que le site la sert, pas telle qu'elle sort du rendu
- **Symptôme** : le 24/09, carré plein refait aux vues d'origine (ADR-0011) : sur le rendu entier, la pièce est complète ; sur l'image recadrée comme le site la sert, le coin du bout arrière de la barre disparaît (6 à 8 px coupés), la barre paraît de nouveau « coupée », ce que le propriétaire venait de refuser. Les contrôles d'habillage étaient à 0 écart.
- **Cause** : deux réglages justes séparément et incompatibles : le cadre de la pièce sur une fiche va jusqu'à 0,62 de la largeur (14/09), le recadrage du site coupe à 0,615 (22/09, « objet montré entier », mesuré sur des barres au chanfrein fixe de 0,6 mm dont le bout arrondi n'atteignait pas le cadre). Les arêtes vives de l'audit (D1) ont amené le coin sur le cadre. Contrôle, vu rouge avant (3 cadres de fiche à 0,62), vert après : chaque cadre de fiche de `rendu_profil.py` tient dans `recadrer_visuels.CADRE` à 0,005 de chaque bord. Corrigé : bord droit 0,61 (pièce 1,8 % plus petite, même vue).
- **Contrôle** : `tests/test_rendus_3d.py::ModelesSource::test_v13_la_fiche_tient_dans_le_recadrage_du_site`
- **Date** : 2026-09-24

### L-064 — séparer la pièce de son ombre se fait sur la couleur, pas sur l'opacité seule
- **Symptôme** : le 24/09, vérification indépendante du large plat : sur toutes les images habillées, fiches et photos studio, le bord de la pièce ressortait 56 à 64 niveaux trop clair en médiane, en escalier — visible dès qu'on ouvre l'image servie de 2400 px (« Agrandir »). Contrôles d'habillage à 0 écart. Le 15/09, le même défaut avait délavé les fils des panneaux de clôture, corrigé famille par famille (`ombre=1.0`), sans remonter à la cause.
- **Cause** : `rendu_sur_blanc()` (habiller.py) séparait la pièce de l'ombre par l'opacité seule (alpha ≥ 250) : chaque pixel du bord anticrénelé, que la pièce ne couvre qu'en partie, était traité en ombre et allégé (× 0,55). Dans le rendu brut, l'ombre est noire et le bord a la couleur de la pièce : la part de pièce d'un pixel se lit (sa luminance divisée par celle de la pièce voisine). Corrigé : `bord_de_piece()` rend cette part à la pièce, seule la part d'ombre est allégée. Mesure `lisere_bord` ajoutée à `controler_rendus.py` (chaque fiche et photo studio servies), vue à +56 à +64 sur les images d'avant, +1 à +3,5 après (bruit du WebP). Rhabillage seul, sans nouveau rendu.
- **Contrôle** : `tests/test_rendus_3d.py::ModelesSource::test_v15_le_bord_de_la_piece_reste_net_sur_le_blanc`
- **Date** : 2026-09-24

### L-065 — quand le poids contredit le nom, chercher quelle cote est fausse avant de masquer le poids
- **Symptôme** : le 24/09, vérification indépendante des tubes ronds : « t 2,5 mm » écrit sur le 26,9x2,5 et « t 3 mm » sur le 88,9x3,00, alors que la page du site officiel dit elle-même 2,35 (description courte, adresse « …269x235mm ») et 3,25 (adresse « …889x325mm »), que le poids de la fiche le confirme (t = 2,33 et 3,22 mm) et que le tableau fournisseur publié (ISO II) aussi. Le poids, en désaccord de 6 et 7 % avec le nom, était masqué depuis le 15/09 : on avait conclu que le poids était faux.
